...
GSEdata<-read.csv("GSE16515_externalGeneNames_mat.csv")
...
pdf("plotsFold2/histogramGSE.pdf")
...
upRegulated_DEGs = fold[filter_combined & fold > 0,]
length(upRegulated_DEGs)
write.csv(upRegulated_DEGs,"upRegulated_DEGs.csv")

downRegulated_DEGs=  fold[filter_combined & fold < 0]
length(downRegulated_DEGs)
